...
import { useRouter } from 'next/navigation'
import dynamic from 'next/dynamic'
import { Menu, User, LogOut, LayoutDashboard, ArrowRight } from 'lucide-react'
import { Button } from '@/components/ui/button'
  ...
  useEffect(() => {
    ...
    const loadProfile = async (user: SupabaseUser) => {
      try {
        ...
        const avatar =
          data?.avatar_url || user.user_metadata?.avatar_url || user.user_metadata?.picture || null

        setProfile({
          first_name: data?.first_name || user.user_metadata?.full_name?.split(' ')[0] || null,
          last_name:
            data?.last_name || user.user_metadata?.full_name?.split(' ').slice(1).join(' ') || null,
          avatar_url: avatar,
        })
      } catch (err) {
        console.error('Profile error:', err)
